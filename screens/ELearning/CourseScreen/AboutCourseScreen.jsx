...
import { ScrollView, View, Text,TouchableOpacity, ActivityIndicator, useWindowDimensions } from "react-native";
  ...
  return (
    <ScrollView style={styles.paragraph} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.rowContainer, globalStyles.verticalTopSpacer10]}>
        ...
        <Text style={[globalStyles.textBlue, styles.extraMargin]}>{courseInfo?.price} LBP</Text>
      </View>
      ...
      <View style={[globalStyles.cardShadow, globalStyles.verticalTopSpacer20]}>
        ...
        <Text style={[globalStyles.textBlue, styles.verticalTopSpacer10, globalStyles.leftText]}>المدرب</Text>
            <FeedbackPersonCard data={{text: courseInfo?.about_teacher, rating: courseInfo?.teacher?.teacher_rating, full_name: courseInfo?.teacher?.full_name, image_absolute_url: `${courseInfo?.formatted_image}${courseInfo?.teacher?.image}`, experienceDomain: courseInfo?.teacher?.experience_domain?.title}} size="large"/>
        </View>
      ...
      <View style={[styles.cardShadow, globalStyles.verticalTopSpacer20]}>
        ...
        <View style={[styles.about, styles.spacing]}>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={()=>{navigation.push('courseCommentsScreen')}}
                    >
                        <Typography
                            color={colors.dark_blue}
                            size={14}
            ...
            {courseInfo?.comment ? <FeedbackCard data={{text: courseInfo?.comment?.comment, rating: courseInfo?.comment?.rating, full_name: courseInfo?.comment?.user?.full_name, image_absolute_url: courseInfo?.comment?.user?.image_absolute_url}} onPress={personPressed} size="small"/> : 
            <View style={styles.aboutLeft}>
            <Typography
                bold={true}
                align="left"
